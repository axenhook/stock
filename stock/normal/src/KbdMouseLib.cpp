...
{
    MY_SEND_MESSAGE(hwnd, WM_KEYDOWN, keycode, 0);
}
...
{
    MY_SEND_MESSAGE(hwnd, WM_KEYUP, keycode, 0);
}
...
void SendKeyString(HWND hwnd, const char *keystr)
{
    while (*keystr != 0)
    {
        MY_SEND_MESSAGE(hwnd, WM_CHAR, *keystr, 0);
        keystr++;
    }
}
...
/* ASCII码对应的按键码 */
//const int ascii2keycode[] = 
//{

//};

/* !"#$%&'()*+,-./09:;<=>?@AZ[\]^_`az{|}~ */
void KeyString(char *pszStr, int strLen)
{
    while (strLen--)
    {
        if ('.' == *pszStr)
        { /* 点 */
            KeyPress(VK_OEM_PERIOD);
        }
...
{
    MY_SEND_MESSAGE(hwnd, WM_MOUSEMOVE, 0, MAKELPARAM(x, y));
}
...
int SendMouseDown(HWND hwnd, int lrm, int x, int y)
{
    switch (lrm)
    {
        case MOUSE_LEFT:
        {
            MY_SEND_MESSAGE(hwnd, WM_LBUTTONDOWN, 0, MAKELPARAM(x, y));
            break;
        }

        case MOUSE_RIGHT:
        {
            MY_SEND_MESSAGE(hwnd, WM_RBUTTONDOWN, 0, MAKELPARAM(x, y));
            break;
        }

        case MOUSE_MIDDLE:
        {
            MY_SEND_MESSAGE(hwnd, WM_MBUTTONDOWN, 0, MAKELPARAM(x, y));
            break;
        }
...
int SendMouseUp(HWND hwnd, int lrm)
{
    switch (lrm)
    {
        case MOUSE_LEFT:
        {
            MY_SEND_MESSAGE(hwnd, WM_LBUTTONUP, 0, 0);
            break;
        }

        case MOUSE_RIGHT:
        {
            MY_SEND_MESSAGE(hwnd, WM_RBUTTONUP, 0, 0);
            break;
        }

        case MOUSE_MIDDLE:
        {
            MY_SEND_MESSAGE(hwnd, WM_MBUTTONUP, 0, 0);
            break;
        }
...
int SendMouseClick(HWND hwnd, int lrm, int x, int y)
{
    switch (lrm)
    {
        ...
        {
            MY_SEND_MESSAGE(hwnd, WM_LBUTTONDOWN, 0, MAKELPARAM(x, y));
            MY_SEND_MESSAGE(hwnd, WM_LBUTTONUP, 0, MAKELPARAM(x, y));
            break;
        }
        ...
        {
            MY_SEND_MESSAGE(hwnd, WM_RBUTTONDOWN, 0, MAKELPARAM(x, y));
            MY_SEND_MESSAGE(hwnd, WM_RBUTTONUP, 0, MAKELPARAM(x, y));
            break;
        }
        ...
        {
            MY_SEND_MESSAGE(hwnd, WM_MBUTTONDOWN, 0, MAKELPARAM(x, y));
            MY_SEND_MESSAGE(hwnd, WM_MBUTTONUP, 0, MAKELPARAM(x, y));
            break;
        }
...
int SendMouseDblClick(HWND hwnd, int lrm, int x, int y)
{
    switch (lrm)
    {
        case MOUSE_LEFT:
        {
            MY_SEND_MESSAGE(hwnd, WM_LBUTTONDBLCLK, 0, MAKELPARAM(x, y));
            break;
        }

        case MOUSE_RIGHT:
        {
            MY_SEND_MESSAGE(hwnd, WM_RBUTTONDBLCLK, 0, MAKELPARAM(x, y));
            break;
        }

        case MOUSE_MIDDLE:
        {
            MY_SEND_MESSAGE(hwnd, WM_MBUTTONDBLCLK, 0, MAKELPARAM(x, y));
            break;
        }
...
void DebugKbdMouseLib(void)
{
    ...
    hwnd = WindowFind(NULL, NULL, "Notepad2");
    if (hwnd != NULL)
    {
        WindowActivate(hwnd);
        KeyPress('1');
...
#if 1 /* 测试发送按键消息, 目前组合按键消息发不成功 */
    /* 找一个TEdit之类的控件做实验 */
    hwnd = (HWND)857040;

    //while (1)
        //SendKeyPress((HWND)133452, VK_F5);
    ...
    /* 会画出两个同心的正方形 */
    hwnd = (HWND)68614;
    /*WindowActivate(hwnd); // 此句不需要打开也可以划出来 */
